...
{% macro create_udf_call_eth_node() %}
    CREATE EXTERNAL FUNCTION IF NOT EXISTS streamline.udf_json_rpc_call(data ARRAY) returns variant api_integration = aws_ethereum_api AS {% if target.name == "prod" %}
