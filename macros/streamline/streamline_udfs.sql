...
{% macro create_udf_get_eth_balances() %}
    CREATE
    OR REPLACE EXTERNAL FUNCTION streamline.udf_get_eth_balances() returns text api_integration = aws_ethereum_api AS {% if target.name == "prod" %}
        'https://e03pt6v501.execute-api.us-east-1.amazonaws.com/prod/bulk_get_uniswap_contracts'
    {% else %}
        'https://mryeusnrob.execute-api.us-east-1.amazonaws.com/dev/bulk_get_uniswap_contracts'
    {%- endif %};
{% endmacro %}
