...
SELECT
    ...
    LOWER(
        flashloan.aave_market
    ) AS aave_market,
    LOWER(
        underlying.aave_token
    ) AS aave_token,
    flashloan.flashloan_quantity / pow(
        10,
        COALESCE(
    ...
    flashloan.flashloan_quantity * COALESCE(
    ...
    flashloan.premium_quantity / pow(
        10,
        COALESCE(
            coalesced_prices.decimals,
            backup_prices.decimals,
    ...
    flashloan.premium_quantity * COALESCE(
        coalesced_prices.coalesced_price,
        backup_prices.price,
        prices_daily_backup.avg_daily_price
    ) / pow(
