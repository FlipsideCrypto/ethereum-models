...
WITH raw_traces AS (
    ...
    WHERE
        1 = 1 {# (
        t.block_number BETWEEN 14000000
        AND 14100000
) #}
),
PARENT AS (
...
FINAL AS (
    ...
    FROM
        final_traces t
        LEFT JOIN ethereum_dev.silver.flat_function_abis f
...
prod AS (
    ...
    WHERE
        1 = 1 {# block_number BETWEEN 14000000
        AND 14100000 #}
    GROUP BY
        ALL
)
SELECT
    ...
    trace_index,
    _call_id,
    array_sort(array_distinct(input_prod)) AS prod_input,
    array_sort(array_distinct(output_prod)) AS prod_output,
    array_sort(array_distinct(input_proposed)) AS proposed_input,
    array_sort(array_distinct(output_proposed)) AS proposed_output
FROM
    prod
