...
complete_dex_swaps AS (
  SELECT
    ...
    CASE
      ...
      WHEN t0.platform = 'uniswap-v4' THEN CONCAT(
        COALESCE(
          c0.symbol,
          CONCAT(SUBSTRING(up.token0, 1, 5), '...', SUBSTRING(up.token0, 39, 42))
        ),
        '-',
        COALESCE(
          c1.symbol,
          CONCAT(SUBSTRING(up.token1, 1, 5), '...', SUBSTRING(up.token1, 39, 42))
        ),
        ' ',
        coalesce(up.fee, 0),
        ' ',
