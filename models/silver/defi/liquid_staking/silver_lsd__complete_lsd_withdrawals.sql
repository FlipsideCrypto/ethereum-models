...
etherfi AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_lsd__etherfi_withdrawals') }}
...
liquidcollective AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_lsd__liquidcollective_withdrawals') }}
...
mantle AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_lsd__mantle_withdrawals') }}
...
origin AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_lsd__origin_withdrawals') }}
...
stakestone AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_lsd__stakestone_withdrawals') }}
