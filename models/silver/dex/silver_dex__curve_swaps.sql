...
,curve_swaps_raw AS (
  SELECT
    ...
    event_inputs
  FROM
    {{ref('silver__logs')}} --change was made here
  WHERE
...
,prices AS (
  ...
  FROM
    {{ref('core__fact_hourly_token_prices')}} -- change was made here
  GROUP BY
    1,
    2
)
SELECT
  ...
  s.amount_in / power(10, COALESCE(dc0.contract_metadata :decimals, tp0.decimals)) AS amount_in,
  s.amount_in / power(10, COALESCE(dc0.contract_metadata :decimals, tp0.decimals)) * tp0.price AS amount_in_usd,
  p1.coins AS token_out,
  s.amount_out / power(10, COALESCE(dc1.contract_metadata :decimals, tp1.decimals)) AS amount_out,
  s.amount_out / power(10, COALESCE(dc1.contract_metadata :decimals, tp1.decimals)) * tp1.price AS amount_out_usd
FROM
  curve_swaps_raw s -- Info for the pool --
  LEFT JOIN {{ref('core__dim_labels')}} l  -- change was made here 
  ON s.pool_address = l.address
  AND l.blockchain = 'ethereum'
  AND l.creator = 'flipside'
  LEFT JOIN {{ref('silver_dex__curve_liquidity_pool')}}
  lp
  ...
  LEFT JOIN {{ref('core__dim_contracts')}} -- changes made here
  dc0
  ...
  LEFT JOIN {{ref('core__dim_contracts')}} -- change was made here
  dc1
