{{ config(
    ...
    unique_key = 'getsubcomponents_reads_id',
    full_refresh = false,
...
function_sigs AS (
    SELECT
        '0xa25e29d3' AS function_sig,
        'getLocalSubComponents' AS function_name
),
inputs AS (
    SELECT
        contract_address,
        ...
        reads_final AS (
            SELECT
                ...
                ARRAY_SLICE(reads_array, 3, ARRAY_SIZE(reads_array)) AS subcomponent_ids
            FROM
                reads_flat
            GROUP BY
                ALL)
            ...
            FROM
                reads_final
            WHERE
                subcomponent_ids IS NOT NULL
                AND subcomponent_ids :: STRING <> '[]'
