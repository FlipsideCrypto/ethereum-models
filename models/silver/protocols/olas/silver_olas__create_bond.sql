...
SELECT
    ...
    ) AS amount_olas_unadj, --OLAS token minted
    amount_olas_unadj / pow(10, 18) :: FLOAT AS amount_olas_adj,
    TRY_TO_NUMBER(
        utils.udf_hex_to_int(
            segmented_data [2] :: STRING
        )
    ) AS token_amount_unadj, --LP token (pair/pool)
    token_amount_unadj / pow(10, 18) :: FLOAT AS token_amount_adj,
    TRY_TO_NUMBER(
        utils.udf_hex_to_int(
